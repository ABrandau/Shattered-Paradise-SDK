...
SpawnPartrollers = function()
	...
	local wolverine1 = Actor.Create("smech", true, { Owner = gdi, Location = GDIPatrol3[1], Facing = Angle.SouthWest })
	...
	local buggy3 = Actor.Create("attackbike", true, { Owner = nod, Location = NodPatrol3[1] })
	local buggy4 = Actor.Create("attackbike", true, { Owner = nod, Location = NodPatrol4[1] })
	...
	Trigger.AfterDelay(DateTime.Seconds(1), function()
		...
		mut.Build(MutantDemoTruck, function(actors)
			Utils.Do(actors, function(actor)
				...
				Trigger.AfterDelay(DateTime.Seconds(5), function()
					carryall.PickupCarryable(actor)
				end)
			end)
		end)
...
SWAssualtGDI = function()
	Trigger.AfterDelay(DateTime.Seconds(11), function()
		NodRadar.ActivateNukePower(SuperWeaponAssualtGDI.Location)
		Trigger.AfterDelay(DateTime.Seconds(36), function()
			MutantTunnelNetwork.ActivateNukePower(SuperWeaponAssualtGDI.Location)
			Trigger.AfterDelay(DateTime.Seconds(20), function()
				SWAssualtGDI()
			end)
		end)
...
WorldLoaded = function()
	...
	Actor.Create("upgrade.unlock_exemplar", true, { Owner = nod })
